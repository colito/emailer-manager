...
class PageHandler extends Operator
{
    ...
    {
        /*$body_content = $this->get_content($uri);
        $this->display_page($body_content);*/
    }
    ...
    public function set_active_template($template_name) { $this->active_template = $template_name; }
    public function get_active_template() { return $this->active_template; }
    ...
    public function determine_active_template($body_content)
    {
        $config = new Config();
        ...
        $this->set_active_template($active_template);

        $results = array();
        $results['body'] = $body_content;
        $results['active_template'] = $active_template;
    ...
    # to display a page.
    public function generate_page($body_content = null, $output_mode = 1)
    {
        # Declarations
        $config = new Config();

        $get_templ = $this->determine_active_template($body_content);

        $body_content = $get_templ['body'];
        $this->active_template = $get_templ['active_template'];
        ...
        $include = $placeholders->replace_placeholders($actual_template, $body_content, $this->page_name, $header_files);
    ...
    public function display_page($body_content)
    {
        $page_output = $this->generate_page($body_content, 1);
